...
ipcMain.on('directory-question', async (event, data) => {
    const options: ("openFile" |
        ...
        "dontAddToRecent")[] = ['openFile', 'openDirectory', 'multiSelections'];
    if (!data) {
    ...
    Downloader.launch({
        ...
        onEvent: {
            onPage: (attributes, total) => {
                ...
                win.webContents.send("downloadUpdatePage", { attributes, total, progress })
            },
            onChapter: (attributes, current, total) => win.webContents.send("downloadUpdateChapter", { attributes, current, total }),
            onVolume: (manga, current, total) => win.webContents.send("downloadUpdateVolume", { manga, current, total })
        }
    }).then((downloader) => {
        function removeDownloadLocations(locations: string | string[] | string[][]): void {
        ...
        ipcMain.on('download', async (event, args: { type: "volume" | "chapter", manga: string, start: number, end?: number, compression: boolean, deleteAfter: boolean }) => {
            ...
            if (type === "volume") {
                if (end) {
                    downloadLocation = await downloader.downloadVolumes(manga, start, end, compression);
                } else {
                    downloadLocation = await downloader.downloadVolume(manga, start, compression);
                }
            } else /* chapter */ {
                if (end) {
                    downloadLocation = await downloader.downloadChapters(manga, start, end, compression);
                } else {
                    downloadLocation = await downloader.downloadChapter(manga, start, compression);
                }
            }
            console.log("Done!");
        ...
        ipcMain.on("openMangaFolder", (event, data) => {
            shell.openPath(path.resolve(path.join(downloader.outputDirectory, data)));
        })
